...
def get_project_files(
    ...
    for file_path in extracted_path.rglob("*"):
        ...
        if extension in DRILL_EXTENSIONS:

            if is_probably_drill(file_path):

                category = "drill"
                layer = "Drill"

        elif extension in GERBER_EXTENSIONS:

            category = "gerber"

        else:

            if is_probably_gerber(file_path):

                category = "gerber"

            elif is_probably_drill(file_path):
        ...
        if category == "gerber":
            ...
            layer = classify_gerber_layer(
                file_path.name
            )

        elif category == "drill":

            layer = classify_drill_layer(
                file_path.name
            )
...
def analyze_project_board(extracted_path):
    """
    Determine PCB board dimensions.

    Priority:
        1. Explicit Board Outline file
        2. Standard outline extensions
        3. Generic Gerber geometry fallback

    Never allow GTL/GBL copper geometry to override a valid
    Board Outline file.
    """
    ...
    for file_path in extracted_path.rglob("*"):
        ...
        # --------------------------------------------------------
        # Determine whether this looks like a Gerber file.
        #
        # Do NOT rely only on the extension because some PCB
        # manufacturers use unusual filenames/extensions.
        # --------------------------------------------------------

        gerber_like = (
            extension.startswith(".g") or
            extension in {
                ".art",
                ".pho",
                ".ger",
        ...
        # --------------------------------------------------------
        # Determine outline status FIRST
        # --------------------------------------------------------

        is_explicit_outline = any(
        ...
        # --------------------------------------------------------
        # If the file is not obviously Gerber and not an outline,
        # skip it.
        # --------------------------------------------------------

        if not gerber_like and not is_explicit_outline:
            continue
        ...
        if is_explicit_outline:
            priority = 100
            method = "explicit_outline_file"

        elif is_outline_extension:
            priority = 90
            method = "outline_extension"

        else:
            priority = 10
            method = "geometry_scan"
    ...
    explicit = [
        ...
        if c["is_explicit_outline"]
    ]

    extension = [
        c for c in candidates
        if (
            c["is_outline_extension"]
            and not c["is_explicit_outline"]
        )
    ]
    ...
    generic = [
        c for c in candidates
        if (
            not c["is_explicit_outline"]
            and not c["is_outline_extension"]
        )
    ]

    # ------------------------------------------------------------
    # Explicit outline ALWAYS wins
    # ------------------------------------------------------------
    ...
    x_min = best.get("x_min")
    x_max = best.get("x_max")
    y_min = best.get("y_min")
    y_max = best.get("y_max")
...
def parse_drill_file(file_path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        for hit in getattr(drill_file, "hits", []) or []:
